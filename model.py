...
class MultimodalLlamaLLM(nn.Module): 

    def get_dummy_token(self, batch_size: int, device: torch.device) -> torch.Tensor:
        return torch.zeros(batch_size, self.image_length, dtype=torch.int64, device=device)
    ...
    def __init__(self, image_length: int, llama, disentangled_length: Optional[int] = None, image_size: int = 768,
                 num_layers: int = 8, mapping_type: MappingType = MappingType.MLP):
        super(MultimodalLlamaLLM, self).__init__()
        self.image_length = image_length
        self.llm = llama 
        self.llm_embedding_size = self.llm.model.embed_tokens.weight.shape[1]
        if mapping_type == MappingType.MLP:
            self.image_project = MLP((image_size, (self.llm_embedding_size * image_length) // 2,
                                     self.llm_embedding_size * image_length))
        else:
            self.image_project = TransformerMapper(image_size, self.llm_embedding_size, image_length,
